export const PRODUCTS = [
    {
      id: 0,
      name: "Donas",
      price: 3000,
      description:
        "Deliciosas donas de diferentes sabores tales como chocolate, fresa, naranja y muchos más",
      image:
        "https://images.unsplash.com/photo-1644845795868-12e95c3268a5?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=300&q=80",
      category: 1,
      stock: 5,
    },
    {
      id: 1,
      name: "Macarrones franceses",
      price: 1000,
      description: "Los pequeños, dulces y coloridos macarrones franceses",
      image:
        "https://images.unsplash.com/photo-1645542563306-ef00dda7f4a0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=300&q=80",
      category: 1,
      stock: 10,
    },
    {
      id: 2,
      name: "Brownie con helado",
      price: 12000,
      description:
        "Delicioso brownie caliente con una bola de helado de vainilla bañado en arequipe",
      image:
        "https://images.unsplash.com/photo-1551024506-0bccd828d307?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=300&q=80",
      category: 1,
      stock: 5,
    },
    {
      id: 3,
      name: "Pie con crema",
      price: 12000,
      description: "pastel relleno con una rica crema pastelera",
      image:
        "https://images.unsplash.com/photo-1543508185-225c92847541?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8N3x8cGllJTIwd2l0aCUyMGNyZWFtfGVufDB8fDB8fA%3D%3D&auto=format&fit=crop&w=500&q=80",
      category: 1,
      stock: 15,
    },
    {
      id: 4,
      name: "Fideos vegetarianos",
      price: 23000,
      description: "fideos vegetarianos salteados con salsa",
      image:
        "https://images.unsplash.com/photo-1647671478150-24f13b5cbc2a?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=664&q=80",
      category: 2,
      stock: 5,
    },
    {
      id: 5,
      name: "Ensalada de verduras",
      price: 25000,
      description: "deliciosa ensalada de verduras acompañada de frutas y queso cottage",
      image:
        "https://images.unsplash.com/photo-1505253716362-afaea1d3d1af?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1587&q=80",
      category: 2,
      stock: 12,
    },
    {
      id: 6,
      name: "Bowl de pollo y verduras salteadas",
      price: 30000,
      description: "Deleita tu paladar con la exquisita combinación de sabores del mediterraneo",
      image:
        "https://images.unsplash.com/photo-1583549322901-9edf5dad0cc4?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8NTF8fHNhbHVkYWJsZXxlbnwwfHwwfHw%3D&auto=format&fit=crop&w=500&q=80",
      category: 2,
      stock: 5,
    },
    {
      id: 7,
      name: "Sandwich de queso con maicitos",
      price: 9000,
      description: "Delicioso sandwich de queso mozzarella con maicitos y salsas de la casa",
      image:
        "https://images.unsplash.com/photo-1528735602780-2552fd46c7af?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1473&q=80",
      category: 3,
      stock: 2,
    },
    {
      id: 8,
      name: "Sandwich de pollo con verduras",
      price: 18000,
      description: "Sandwich con pan artesanal relleno con pollo y deliciosas verduras como pepinillos, cebolla y lechuga",
      image:
        "https://images.unsplash.com/photo-1559466273-d95e72debaf8?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=688&q=80",
      category: 3,
      stock: 8,
    },
    {
      id: 9,
      name: "Sandwich qbano",
      price: 16000,
      description: "Sandwich de jamones en pan francés",
      image:
        "https://images.unsplash.com/photo-1554433607-66b5efe9d304?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=764&q=80",
      category: 3,
      stock: 5,
    },
    {
      id: 10,
      name: "Hamburguesa de queso doble carne",
      price: 15000,
      description: "Deliciosa hamburguesa de con doble carne angus 100% de 120gr y queso cheddar",
      image:
        "https://images.unsplash.com/photo-1614891669421-964261109bb4?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1471&q=80",
      category: 4,
      stock: 5,
    },
    {
      id: 11,
      name: "Hamburguesa de pollo",
      price: 19000,
      description: "Hamburguesa de pollo apanado con pan artesanal",
      image:
        "https://images.unsplash.com/photo-1615297928064-24977384d0da?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1112&q=80",
      category: 4,
      stock: 10,
    },
    {
      id: 12,
      name: "Hamburguesa sencilla",
      price: 11000,
      description: "Una hambueguesa sencilla pero con un sabor exquisito que siempre querrás repetir",
      image:
        "https://images.unsplash.com/photo-1593022754339-f23454332fd8?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80",
      category: 4,
      stock: 5,
    },
    {
      id: 13,
      name: "Pizza margarita",
      price: 18000,
      description: "Deliciosa pizza de queso con tomate",
      image:
        "https://images.unsplash.com/photo-1594078187393-71dd5b8833f0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80",
      category: 5,
      stock: 20,
    },
    {
      id: 14,
      name: "Fiesta peperoni",
      price: 22000,
      description: "Pizza con doble queso mozzarella y doble porción de peperoni",
      image:
        "https://images.unsplash.com/photo-1541745537411-b8046dc6d66c?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=688&q=80",
      category: 5,
      stock: 5,
    },
    {
      id: 15,
      name: "Pizza 3 quesos",
      price: 24000,
      description: "Pizza con doble queso mozzarella, queso azul y queso crema",
      image:
        "https://images.unsplash.com/photo-1571407970349-bc81e7e96d47?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1025&q=80",
      category: 5,
      stock: 1,
    },
  ];